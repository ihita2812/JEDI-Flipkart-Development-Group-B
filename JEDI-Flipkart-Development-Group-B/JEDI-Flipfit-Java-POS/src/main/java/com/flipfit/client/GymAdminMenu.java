             ...
             try{
                 ...
                 switch (choice) {
                     ...
                     case 3:
                         ...
                         break;
// REPLACE the entire "case 4:" block with this code

                     case 4:
                         List<GymOwner> allOwners = admin.getAllOwners();

                         System.out.printf("%n**************************** ALL GYM OWNERS & THEIR GYMS ****************************%n");

                         if (allOwners.isEmpty()) {
                             System.out.println("\n-- There are no gym owners registered on the platform. --\n");
                         } else {
                             for (GymOwner owner : allOwners) {
                                 // Print the main owner details
                                 System.out.println("\n=====================================================================================");
                                 System.out.printf(" Owner ID: %-4d | Name: %-20s | Username: %-15s\n",
                                         owner.getOwnerId(),
                                         owner.getName(),
                                         owner.getUserName());
                                 System.out.println("-------------------------------------------------------------------------------------");

                                 // Get the nested list of gyms for this specific owner
                                 List<GymCenter> gyms = owner.getGyms();

                                 if (gyms == null || gyms.isEmpty()) {
                                     System.out.println("   -> This owner has not registered any gyms yet.");
                                 } else {
                                     System.out.println("   Registered Gyms:");
                                     // Loop through and print the details of each gym
                                     for (GymCenter gym : gyms) {
                                         String status = switch (gym.getApprovalStatus()) {
                                             case 0 -> "Rejected";
                                             case 1 -> "Approved";
                                             default -> "Pending";
                                         };
                                         System.out.printf("     -> Gym ID: %-4d | Name: %-25s | Status: %s\n",
                                                 gym.getCenterId(),
                                                 gym.getName(),
                                                 status);
                                     }
